...
type Book = { writerId: number; title: string; type: string; numberOfWords: number };

const getAmountPublishedInFantasy = (writers: Writer[], books: Book[], authorName: string): number => {
  const author = writers.find((writer) => writer.name === authorName);
  if (!author) return 0;

  const authorBooks = books.filter((book) => book.writerId === author.id && book.type === "fantasy");

  const totalWordsInFantasyBooks = authorBooks.reduce((total, book) => total + book.numberOfWords, 0);

  return totalWordsInFantasyBooks;
}
